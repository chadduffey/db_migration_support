...
def _update_stats(ext_type, size, testing_mode=False):
...
def dropbox_stats(dbx, member, path, testing_mode=False):
	""" Gather stats for the Dropbox account. 
	"""
	try:
		dir_listing = dbx.as_user(member).files_list_folder(path)

		for item in dir_listing.entries:
			
			if type(item) == dropbox.files.FileMetadata: 
				#print(". {}".format(item.name))
				_update_stats(_extension_type(item), item.size, testing_mode)

			if type(item) == dropbox.files.FolderMetadata: 
				print("{}".format(item.path_display))
				if not testing_mode:
					dropbox_stats(dbx, member, item.path_display, testing_mode)
	except:		
		if path == "":
			path = "{folder root}"
